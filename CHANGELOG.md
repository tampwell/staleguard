...
- The relock story. When a lockfile regenerates, the tool window now tells you what actually moved: version movements with the advisories they fixed or INTRODUCED, arrivals, and departures. A relock that introduces a known vulnerability raises a warning notification naming the advisory and the movement that brought it in. First sighting of a lockfile is a silent baseline, only a real change becomes news, and the story survives an IDE restart, so a relock is news exactly once.
- buildSrc constants are editable now. The pre-catalog Versions object idiom (const val gson = "2.10.1") joins the batch update dialog, Apply All Patch, and the Alt+Enter editor fix as a first-class citizen: the same pattern that reads a constant locates the exact text span to replace, several dependencies sharing one constant merge to the highest selected suggestion, a blast-radius confirmation fires when one constant feeds several declarations, and a constant that moved since the dialog opened is left alone rather than blindly overwritten. Templated and non-const values stay read-only, honestly.
- Lock drift is ambient. The status bar count now includes locks out of step with the declarations, computed by the same logic as the tool window so the surfaces can never disagree, and Kotlin Multiplatform source-set blocks are covered by a pinned regression test in both idioms.

## [2.6.0] - 2026-09-09

### Added
- The lockfile is the truth. When a project uses Gradle dependency locking, Staleguard now reads every lockfile (current single-file format and the legacy per-configuration directory both), warns when a lock is out of step with what the build files declare (the forgot-to-relock footgun), and runs the vulnerability check on the LOCKED versions, the most exact input it can get. The tool window shows both, and every row double-clicks straight to the exact lockfile line. Because applying an update in a locked module creates that drift the moment it lands, the batch applier now says so itself and names the --write-locks follow-up. Staleguard never edits a lockfile.
...
### Added
- Transitive vulnerability sweep. Every artifact your resolved Maven tree pulls in beyond the declared dependencies is checked against the OSV database, and each hit appears in the tool window with the path that brings it in, so a vulnerable transitive names the dependency to change. Same 24 hour cache and offline behavior as the declared checks, evicted versions are skipped because a CVE claim about a jar you do not run is noise, and Gradle builds are excluded honestly rather than guessed at.
...
### Added
- Upgrade rehearsal. Check Upgrade Impact now also swaps the candidate jar into every module's classpath and resolves everything again, so the verdict covers what no per-member check can: whether the upgrade fixes the linkage problems the classpath doctor already knows about, and whether it introduces new ones anywhere, in any module, production or tests, including breaks between two OTHER jars that only this version change exposes. The report says "fixes 2 known linkage problems" or "introduces 1 new" with the exact callers named, in the dialog and in the Markdown paste.
- THE FIX applies itself. One button in the findings dialog bumps every fixable jar where it is declared, through the same battle-tested machinery the batch updater uses: literals, properties, version catalogs, gradle.properties. A jar that arrived transitively has no declaration to edit, so Maven builds get a dependencyManagement pin added to the root pom (the tool-blessed way to govern a transitive version), and Gradle builds get the exact constraints block on the clipboard, because inserting text into your build script would be a guess about your formatting, not an edit. Every outcome is reported by name; nothing happens silently.
- The verdict survives restarts. The last audit's result is persisted per project, so a reopened project shows its linkage health and editor warnings immediately instead of nothing until the first re-check, which still runs and corrects anything that changed while the IDE was closed.
- Rehearsal steers the batch dialog. An upgrade whose rehearsal introduces linkage problems elsewhere on the classpath is deselected and capped in confidence exactly like one that breaks your own calls, and the editor's measured note says why: your code survives, the classpath does not.
- The doctor warns in the editor. A dependency whose RESOLVED version breaks other jars' calls now gets an inspection warning right at the declaration, in all four build-file surfaces (pom, Gradle, Kotlin DSL, version catalogs), naming how many calls fail and whose. When the audit has computed THE FIX, the warning carries a one-click bump to that version. Highlighting only reads the verdict the audits maintain; it never scans a classpath.
- The doctor is always on. One quiet audit runs shortly after the project opens (the watcher previously waited for the first sync), and the verdict stays visible without opening anything: the status bar count gains "N linkage problems" alongside updates and vulnerabilities, and the tool window summary carries the classpath's health as of the last check. Every audit, whether explicit, watched, or at startup, updates the same ambient verdict.
