...
- buildSrc constants are editable now. The pre-catalog Versions object idiom (const val gson = "2.10.1") joins the batch update dialog and Apply All Patch as a first-class citizen: the same pattern that reads a constant locates the exact text span to replace, several dependencies sharing one constant merge to the highest selected suggestion, and a constant that moved since the dialog opened is left alone rather than blindly overwritten. Templated and non-const values stay read-only, honestly.
